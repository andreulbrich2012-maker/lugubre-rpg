...
router.get('/races', async (_, res) => {
  const result = await tryQuery('select * from races where deleted_at is null order by name');
  res.json(result?.rows || await getLocalCatalog('races'));
});

router.get('/classes', async (_, res) => {
  const result = await tryQuery('select * from classes where deleted_at is null order by name');
  res.json(result?.rows || await getLocalCatalog('classes'));
});

router.get('/origins', async (_, res) => {
  const result = await tryQuery('select * from origins where deleted_at is null order by name');
  res.json(result?.rows || await getLocalCatalog('origins'));
});

router.get('/skills', async (_, res) => {
  const result = await tryQuery('select id, "key", name, attribute from skills where deleted_at is null order by name');
